...
<div class="mb-8 flex gap-1.5 overflow-x-auto snap-x snap-mandatory pb-2 week-scroll">
	{#each weekDays.days as day}
		...
		<div
			class="flex-shrink-0 snap-center rounded-xl overflow-hidden flex flex-col transition-all {hasContent ? 'w-[85vw] md:w-0 md:flex-1 md:aspect-[3/4] md:max-h-[300px]' : 'w-[60px] md:w-[60px] md:flex-none md:aspect-[3/4] md:max-h-[300px]'} {dropDayDate === dayIso ? 'border border-accent bg-accent/5' : day.isToday ? 'border border-text/30 bg-surface/50' : hasContent ? 'border border-border bg-surface/20' : 'bg-surface/10'}"
	...
	{#if weekDays.later.length > 0}
		<div class="flex-shrink-0 w-[85vw] md:w-0 md:flex-1 snap-center border border-border rounded-xl overflow-hidden flex flex-col md:aspect-[3/4] md:max-h-[300px] bg-surface/20">
